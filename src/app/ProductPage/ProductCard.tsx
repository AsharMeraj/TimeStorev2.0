    ...
    return (
        <div className='w-fit m-auto focus:outline-none hover:scale-105 duration-200 cursor-pointer'>
            <Link href={`/AllProducts/${data.id}`} prefetch={false}>
                <Image className='w-[16rem] sm:w-[18rem] bg-black/5 border-b-4 border-[var(--primary)] m-auto' alt='' width={600} height={600} src={`${data.Image[0].url}`} />
                <h2 className=' w-[16rem] sm:w-[16rem] font-semibold text-[1rem] max-[840px]:text-[1rem] mt-4'>
                    {data.Name}
                </h2>
