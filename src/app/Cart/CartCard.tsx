                            ...
                            <div className='flex flex-col h-[8rem] max-[558px]:h-[7rem] max-[300px]:h-fit justify-between w-full max-[920px]:gap-2 max-[460px]:gap-0 ml-[1.5rem] max-[558px]:ml-[0.5rem]'>
                                <main className='flex flex-col gap-2 '>
                                    <div className='flex w-full max-[558px]:min-w-full items-center justify-between'>
                                        <h2 className='md:text-[1.3rem] font-bold text-black/70 text-base sm:text-base leading-snug'>{item.data.Name}</h2>
                                        <p className='text-black/60 font-bold text-right text-[0.9rem] sm:text-base'>MRP : ${item.data.Price.toFixed(1)}</p>
                                    </div>
                                    <h2 className='font-semibold text-[rgb(150,150,150)] sm:text-base text-[13px]'>{item.data.Category}</h2>
                                </main>
                                    ...
                                    <main className='flex max-[460px]:flex-col max-[460px]:gap-0 w-fit gap-4'>
                                        <div className='flex gap-1'>
                                            <div className='flex items-center'>
                                                <h2 className='text-black/70 font-semibold sm:text-base text-[0.9rem]'>Size:</h2>
                                            </div>
                                            <select
                                                onChange={(e) => updateCartItems(e, 'selectedSize', item.data)}
                                                className='cursor-pointer sm:text-base text-[0.8rem] focus:outline-none hover:text-black'
                                        ...
                                        <div className='flex gap-2'>
                                            <div className='flex items-center'>
                                                <h2 className='text-black/70 font-semibold sm:text-base text-[0.8rem]'>Quantity:</h2>
                                            </div>
                                            <select
                                                onChange={(e) => updateCartItems(e, 'quantity', item.data)}
                                                className='p cursor-pointer focus:outline-none sm:text-base text-[0.8rem] hover:text-black'
